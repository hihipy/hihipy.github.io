...
SELECT
    ...
    printf('%,d', CAST(p.total_cost AS INTEGER))  AS "Project Total Cost ($)",
    printf('%,d', (SELECT CAST(SUM(total_cost_ic) AS INTEGER)
                   FROM project_funders
...
-- analysis is 3,580.
SELECT
    printf('%,d', COUNT(*))  AS "Projects Without Funder Data"
FROM projects p
WHERE NOT EXISTS (
    SELECT 1
