...
[Run this query in Datasette Lite](https://lite.datasette.io/?url=https://pgbd.casa/data/penobscot-nclex.sqlite#/penobscot-nclex?sql=WITH+first_failers+AS+%28%0A++SELECT+a.student_id%2C+t.ordinal+AS+fail_ord%0A++FROM+attempts+a%0A++JOIN+term_order+t+ON+t.cohort+%3D+a.testing_cohort%0A++WHERE+a.attempt_number+%3D+1+AND+a.result+%3D+0%0A%29%2C%0Astrict_retakers+AS+%28%0A++SELECT+DISTINCT+ff.student_id%0A++FROM+first_failers+ff%0A++JOIN+attempts+a2++ON+a2.student_id+%3D+ff.student_id%0A++JOIN+term_order+t2+ON+t2.cohort+%3D+a2.testing_cohort%0A++WHERE+t2.ordinal+%3E+ff.fail_ord%0A%29%2C%0Astrict_eventually_passed+AS+%28%0A++SELECT+DISTINCT+ff.student_id%0A++FROM+first_failers+ff%0A++JOIN+attempts+a2++ON+a2.student_id+%3D+ff.student_id%0A++JOIN+term_order+t2+ON+t2.cohort+%3D+a2.testing_cohort%0A++WHERE+t2.ordinal+%3E+ff.fail_ord+AND+a2.result+%3D+1%0A%29%0ASELECT%0A++%28SELECT+COUNT%28%2A%29+FROM+first_failers%29+++++++++++++++++++++++++++++++++++++AS+%22First-Time+Failures%22%2C%0A++%28SELECT+COUNT%28%2A%29+FROM+strict_retakers%29+++++++++++++++++++++++++++++++++++++AS+%22Retook%22%2C%0A++%28SELECT+COUNT%28%2A%29+FROM+first_failers%29+-+%28SELECT+COUNT%28%2A%29+FROM+strict_retakers%29+AS+%22Did+Not+Retake%22%2C%0A++ROUND%28100.0+%2A+%28SELECT+COUNT%28%2A%29+FROM+strict_retakers%29%0A++++++++++++++++%2F+%28SELECT+COUNT%28%2A%29+FROM+first_failers%29%2C+2%29+++++++++++++++++++++++++AS+%22%25+Retook%22%2C%0A++%28SELECT+COUNT%28%2A%29+FROM+strict_eventually_passed%29+++++++++++++++++++++++++++++++++AS+%22Eventually+Passed%22%2C%0A++ROUND%28100.0+%2A+%28SELECT+COUNT%28%2A%29+FROM+strict_eventually_passed%29%0A++++++++++++++++%2F+%28SELECT+COUNT%28%2A%29+FROM+strict_retakers%29%2C+2%29+++++++++++++++++++++++++AS+%22Conversion+%25%22%3B)
...
data: {
  ...
  datasets: [{
    ...
    pointBorderColor: ['#0969DA','#BF8700','#0969DA','#0969DA'],
    borderWidth: 0
  }]
},
options: {
  ...
  plugins: {
    ...
    tooltip: { callbacks: { label: function(ctx) { return ctx.parsed.x + ' students'; } } }
  },
  scales: {
    x: { min: 0, max: 1000, title: { display: true, text: 'Students' } },
...
[Run this query in Datasette Lite](https://lite.datasette.io/?url=https://pgbd.casa/data/penobscot-nclex.sqlite#/penobscot-nclex?sql=WITH+first_failers+AS+%28%0A++SELECT+a.student_id%2C+a.testing_cohort+AS+fail_cohort%2C+t.ordinal+AS+fail_ord%0A++FROM+attempts+a%0A++JOIN+term_order+t+ON+t.cohort+%3D+a.testing_cohort%0A++WHERE+a.attempt_number+%3D+1+AND+a.result+%3D+0%0A%29%2C%0Astrict_retakers+AS+%28%0A++SELECT+DISTINCT+ff.student_id%2C+ff.fail_cohort%2C+ff.fail_ord%0A++FROM+first_failers+ff%0A++JOIN+attempts+a2++ON+a2.student_id+%3D+ff.student_id%0A++JOIN+term_order+t2+ON+t2.cohort+%3D+a2.testing_cohort%0A++WHERE+t2.ordinal+%3E+ff.fail_ord%0A%29%0ASELECT%0A++ff.fail_cohort+++++++++++++++++++++++++++++++++++++++++++AS+%22Testing+Cohort%22%2C%0A++COUNT%28%2A%29++++++++++++++++++++++++++++++++++++++++++++++++AS+%22First-Time+Failures%22%2C%0A++COUNT%28%2A%29+-+COUNT%28sr.student_id%29++++++++++++++++++++++++++AS+%22Did+Not+Retake%22%2C%0A++ROUND%28100.0+%2A+%28COUNT%28%2A%29+-+COUNT%28sr.student_id%29%29+%2F%0A++++++++++++++++COUNT%28%2A%29%2C+2%29+++++++++++++++++++++++++++++++++AS+%22%25+Did+Not+Retake%22%0AFROM+first_failers+ff%0ALEFT+JOIN+strict_retakers+sr+ON+sr.student_id+%3D+ff.student_id%0AGROUP+BY+ff.fail_cohort%2C+ff.fail_ord%0AORDER+BY+ff.fail_ord%3B)
...
[^ncsbn-data]: [NCSBN data](https://www.ncsbn.org/exams/exam-statistics-and-publications/nclex-pass-rates.page).
