---
layout: "single"
title: "~/puerta  # Résumé PDF"
description: "Philip Gregory Bachas-Daunert. Printable résumé."
summary: "Downloadable PDF résumé."
draft: false
showAuthor: false
showDate: false
showReadingTime: false
showWordCount: false
showBreadcrumbs: true
showTableOfContents: false
showPagination: false
---

{{< lead >}}
{{< typeit speed=70 loop=true breakLines=false >}}
Mi Currículum
My Résumé
El Meu Currículum
Το Βιογραφικό μου
{{< /typeit >}}
{{< /lead >}}

A printable version of my résumé. The sala has the readable, expanded version with more detail; this PDF is the formal one-pager for recruiters and applications.

<div class="my-6">
  <object data="/Philip%20Bachas-Daunert%20Resume.pdf" type="application/pdf" width="100%" style="height: 90vh; min-height: 600px; border: 1px solid var(--color-neutral-300, #e5e5e5); border-radius: 0.375rem;">
    <p>Your browser does not support inline PDFs. <a href="/Philip%20Bachas-Daunert%20Resume.pdf">Click here to download the résumé</a>.</p>
  </object>
</div>
