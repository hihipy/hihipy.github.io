...
(function () {
  ...
  function applyTheme(chart, target) {
    ...
    if (chart.options.scales) {
      Object.keys(chart.options.scales).forEach(function (k) {
        ...
        if (s.title) s.title.color = palette.text;
      });
    }
  ...
  function sync() {
    ...
    getCharts().forEach(function (c) { applyTheme(c, t); });
    currentTheme = t;
  }

  function init() {
    if (typeof Chart === 'undefined') { setTimeout(init, 500); return; }
    var charts = getCharts();
    if (charts.length === 0) { setTimeout(init, 500); return; }
